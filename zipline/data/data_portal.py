...
class DataPortal(object):
    ...
    future_minute_reader : BcolzFutureMinuteBarReader, optional
        ...
        minute data backtests or minute history calls. This can be used
        to serve daily calls if no daily bar reader is provided.
    option_daily_reader : BcolzDailyChainReader, optional
        The daily chain reader for options. This will be used to service
        daily data backtests or daily history calls in a minute backetest.
    ...
    def __init__(
        ...
        aligned_option_session_reader = self._ensure_reader_aligned(option_daily_reader)
        ...
        if aligned_future_minute_reader is not None:
            ...
            aligned_minute_readers[ContinuousFuture] = ContinuousFutureMinuteBarReader(
                aligned_future_minute_reader, self._roll_finders
            )
        ...
        if aligned_future_session_reader is not None:
            ...
            self._roll_finders["volume"] = VolumeRollFinder(
                self.trading_calendar, self.asset_finder, aligned_future_session_reader
            )
            aligned_session_readers[
                ContinuousFuture
            ] = ContinuousFutureSessionBarReader(
                aligned_future_session_reader, self._roll_finders
            )

        if aligned_option_session_reader is not None:
            aligned_session_readers[Option] = aligned_option_session_reader
    ...
    def get_spot_value(self, assets, field, dt, data_frequency):
        ...
        if assets_is_scalar:
            return self._get_single_asset_value(
                session_label, assets, field, dt, data_frequency
            )
        else:
    ...
    def _get_history_minute_window(self, assets, end_dt, bar_count, field_to_use):
        ...
        asset_minute_data = self._get_minute_window_data(
            assets, field_to_use, minutes_for_window
        )
        ...
        if field == "price":
            ...
            for asset in df.columns[assets_with_leading_nan]:
                last_traded = self.get_last_traded_dt(
                    asset, history_start, ffill_data_frequency
                )
    ...
    def _get_minute_count_for_transform(self, ending_minute, days_count):
        ...
        completed_sessions = cal.sessions_window(
            cal.previous_session_label(ending_session), 2 - days_count
        )

        completed_sessions_minute_count = self.trading_calendar.minutes_count_for_sessions_in_range(
    ...
    def get_simple_transform(
        self, asset, transform_name, dt, data_frequency, bars=None, field=None
    ):
        if transform_name == "returns":
            ...
            hst = self.get_history_window(
                ...
                2,
                "1d",
                "price" if field is None else field,
                data_frequency,
                ffill=True,
            )[asset]
        ...
        if transform_name == "annualized_volatility":
            ann_vol = AnnualizedVolatility()
            returns = price_arr / price_arr.shift(1) - 1
            out = np.empty(shape=(1,), dtype=np.float64)
            ann_vol.compute(dt, [asset], out, returns, 252)
            return out[0]
        elif transform_name == "mavg":
