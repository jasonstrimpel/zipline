...
DAYS_IN_MONTH = 30
MONTHS = [0, 1, 2, 6, 1 * 12, 2 * 12, 3 * 12, 5 * 12, 7 * 12, 10 * 12, 20 * 12, 30 * 12]

terms = [DAYS_IN_MONTH * x for x in MONTHS]
new_terms = [i for i in range(0, max(terms) + 1)]
...
def get_interpolated_rate(
    rates, days_to_expiration, kind="linear"
):
    """ Interpolate rates out to 30 years

    """
    # insert 0.0 as immediate overnight rate
    rates = np.insert(rates, 0, 0.0)
    f = interp1d(terms, rates, kind=kind)
    ff = f(new_terms)
    return ff[days_to_expiration]
