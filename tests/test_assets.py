...
class TestOption(WithAssetFinder, ZiplineTestCase):
    ...
    def init_class_fixtures(cls):
        ...
        cls.future = cls.asset_finder.lookup_option_symbol('IBM200409C00106000')
        cls.future2 = cls.asset_finder.lookup_option_symbol('IBM200409P00106000')

    def test_repr(self):
        reprd = repr(self.option)
        self.assertEqual("Option(2468 [IBM200409C00106000])", reprd)
    ...
    def test_to_and_from_dict(self):
        dictd = self.option.to_dict()
        for field in _options_defaults.keys():
            self.assertTrue(field in dictd)

        from_dict = Option.from_dict(dictd)
        self.assertTrue(isinstance(from_dict, Option))
        self.assertEqual(self.option, from_dict)

    def test_root_symbol(self):
        self.assertEqual('IBM', self.option.root_symbol)

    def test_lookup_option_symbol(self):
        """
        Test the lookup_future_symbol method.
        """
        om = TestFuture.asset_finder.lookup_option_symbol('IBM200409C00106000')
        self.assertEqual(om.sid, 2468)
        self.assertEqual(om.symbol, 'IBM200409C00106000')
        self.assertEqual(om.root_symbol, 'IBM')
        self.assertEqual(om.notice_date, pd.Timestamp('2014-01-20', tz='UTC'))
        self.assertEqual(om.expiration_date,
                         pd.Timestamp('2014-02-20', tz='UTC'))
        self.assertEqual(om.auto_close_date,
        ...
        cl = TestFuture.asset_finder.lookup_option_symbol('IBM200409P00106000')
        self.assertEqual(cl.sid, 0)
        self.assertEqual(cl.symbol, 'IBM200409P00106000')
        self.assertEqual(cl.root_symbol, 'IBM')
        self.assertEqual(cl.start_date, pd.Timestamp('2005-12-01', tz='UTC'))
        self.assertEqual(cl.notice_date, pd.Timestamp('2005-12-20', tz='UTC'))
        self.assertEqual(cl.expiration_date,
                         pd.Timestamp('2006-01-20', tz='UTC'))
        ...
        with self.assertRaises(SymbolNotFound):
            TestFuture.asset_finder.lookup_option_symbol('')

        with self.assertRaises(SymbolNotFound):
            TestFuture.asset_finder.lookup_option_symbol('#&?!')

        with self.assertRaises(SymbolNotFound):
            TestFuture.asset_finder.lookup_option_symbol('FOOBAR')

        with self.assertRaises(SymbolNotFound):
            TestFuture.asset_finder.lookup_option_symbol('XXX99')
